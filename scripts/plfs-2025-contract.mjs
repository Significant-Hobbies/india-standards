...
export const PLFS_2025_CONTRACT = Object.freeze({
  ...
  design: {
    methodologySource: {
      relativePath:
        "data/sources/plfs-2025/PLFS_changes_in_2025_Final.pdf",
      sha256:
...
export async function inspectPlfsSource(
  ...
  for await (const chunk of createReadStream(sourcePath)) {
    ...
    if (!headerComplete) {
      ...
      if (newlineIndex === -1) {
        ...
        headerBuffer = Buffer.concat([headerBuffer, chunk.subarray(0, newlineIndex)]);
        headerComplete = true;
      }
    }
  }
  ...
  if (sha256 !== contract.sha256) {
    errors.push(`Checksum mismatch: expected ${contract.sha256}, got ${sha256}.`);
  }
