...
function changedSourceFiles() {
  ...
  for (const args of [
    ...
    for (const file of run("git", args).stdout.split("\n")) {
      if (file) {
        candidates.add(file);
      }
    }
  }
  return [...candidates].filter((file) => sourceExtensions.has(extname(file)));
}

function checkFormat() {
  const files = changedSourceFiles();
  if (files.length === 0) {
    console.log("Format boundary: no changed JavaScript or TypeScript files.");
    return;
  }
  run("pnpm", [
    "exec",
  ...
  console.log(`Format boundary: ${files.length} changed source files pass.`);
}
...
function checkUnused() {
  ...
  console.log(
    `Unused: files=${observed.files}, exports=${observed.exports}, ` +
      `types=${observed.types}, dependencies=${observed.dependencies}, ` +
      `devDependencies=${observed.devDependencies}, unlisted=${observed.unlisted}, ` +
...
function checkDuplication() {
  const outputDirectory = mkdtempSync(join(tmpdir(), "india-standards-jscpd-"));
  run("pnpm", [
    "exec",
    "jscpd",
    ...productionPaths,
    ...
    "strict",
    "--ignore",
    "**/*.test.*,**/*.d.mts,**/node_modules/**,**/.next/**,**/.open-next/**,**/dist/**,**/build/**,scripts/check-code-health.mjs",
    "--reporters",
    "json",
    "--output",
    outputDirectory,
  ...
  console.log(
    `Duplication: ${observed.clones} groups, ` +
      `${observed.duplicatedLines}/${observed.lines} lines ` +
...
function checkCycles() {
  ...
  if (cycles.length > 0) {
    throw new Error(`Dependency cycles detected: ${cycles.length}`);
  }
...
function checkDependencies() {
  ...
  const critical = severe.filter(
    (advisory) => advisory.severity === "critical"
  );
  const high = severe.filter((advisory) => advisory.severity === "high");
  console.log(
...
function checkSuppressions() {
  ...
  files.push(
    resolve(projectRoot, "agent-edge.mjs"),
    resolve(projectRoot, "worker.mjs")
  );
  const matches = files
